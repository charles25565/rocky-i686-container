FROM ghcr.io/charles25565/rocky-i686-selfhost-test:latest AS installer 
COPY i686.repo /etc/yum.repos.d
RUN dnf install --setopt=tsflags=noscripts --skip-broken --repo=rocky-devel --releasever=9 --installroot=/out --forcearch=i686 $(rpm -qa --queryformat '%{NAME} ') -y
